...
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[])
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
    	...
    	return DS18X20_OK;
	        ...
	        debug_print( "DS18X20_start_meas: Short Circuit !\r" );
	    #endif
...
//EEPROMSUPPORT untested on photon 
    ...
    uint8_t DS18X20_write_scratchpad( uint8_t id[], uint8_t th, uint8_t tl, uint8_t conf)
    {
        ...
        if( ow_input_pin_state() ) { // only send if bus is "idle" = high
            ...
            #ifdef DS18X20_VERBOSE
                debug_print( "DS18X20_write_scratchpad: Short Circuit !\r" );
            #endif
	...
	uint8_t DS18X20_read_scratchpad( uint8_t id[], uint8_t sp[] )
	{
    	...
    	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
    	    ...
    	    #ifdef DS18X20_VERBOSE
    	        debug_print( "DS18X20_read_scratchpad: Short Circuit !\r" );
    	    #endif
    	    return DS18X20_ERROR;
    	}
	}
	...
	uint8_t DS18X20_copy_scratchpad( uint8_t with_power_extern, uint8_t id[] )
	{
	    ...
	    if( ow_input_pin_state() ) { // only send if bus is "idle" = high
	        ...
	        #ifdef DS18X20_VERBOSE
	            debug_print( "DS18X20_copy_scratchpad: Short Circuit !\r" );
	        #endif
	        return DS18X20_START_FAIL;
	    }
	}
	...
	uint8_t DS18X20_recall_E2( uint8_t id[] )
	{
	    ...
	    if( ow_input_pin_state() ) { // only send if bus is "idle" = high
	        ...
	        // handling
	        delay(DS18X20_COPYSP_DELAY);
	        return DS18X20_OK;
	    } else {
	    #ifdef DS18X20_VERBOSE
	        debug_print( "DS18X20_recall_E2: Short Circuit !\r" );
	    #endif
	    return DS18X20_ERROR;
	    }
	}
